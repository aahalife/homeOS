...
public final class SkillIntegrationService: @unchecked Sendable {
    ...
    public func parseSkillInvocation(from toolCall: ToolCall) -> SkillInvocationRequest? {
        ...
        let rawParams = args["parameters"] as? [String: Any] ?? args

        guard let skillId = skillId else { return nil }

        // Convert parameters to [String: String]
        ...
        for (key, value) in rawParams {
            if let stringValue = value as? String {
                ...
                parameters[key] = numberValue.stringValue
            } else {
    ...
    func generateSkillToolSpecs() -> [Tool] {
        ...
        return skills.map { skill in
            Tool(
                ...
                function: ToolFunction(
                    ...
                    parameters: generateSkillParametersJSON(skill)
                )
            )
        }
    }

    private func generateSkillParametersJSON(_ skill: SkillDefinition) -> JSONValue? {
        ...
        for step in skill.toolSequence {
            for param in step.params {
                if properties[param] == nil {
                    properties[param] = .object([
                        "type": .string("string"),
                        "description": .string("Parameter for \(step.tool)")
                    ])
                }
            }
        }
...
public struct SkillInvocationRequest: Sendable {
    ...
    public init(skillId: String, parameters: [String: String], toolCallId: String) {
        self.skillId = skillId
...
extension ChatTurn {
    private static nonisolated(unsafe) var skillExecutionKey: UInt8 = 0
